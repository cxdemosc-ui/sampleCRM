/*******************************************************
 * SampleCRM - FINAL script.js for Aug 2025
 * - Stable rendering for customer, cards, SRs
 * - Full auto-refresh after updates
 * - Card actions robust & disables as needed
 * - SR Update/Close mapped safely
 *******************************************************

const SUPABASE_PROJECT_REF = 'yrirrlfmjjfzcvmkuzpl';
const API_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InlyaXJybGZtampmemN2bWt1enBsIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTMxODk1MzQsImV4cCI6MjA2ODc2NTUzNH0.Iyn8te51bM2e3Pvdjrx3BkG14WcBKuqFhoIq2PSwJ8A'; // <-- Use your real key!
...
async function fetchCustomer(identifier, searchType='auto') {
  ...
  const r = await fetch(ENDPOINTS.getCustomer, { method:'POST', headers:{apikey:API_KEY, Authorization:`Bearer ${API_KEY}`,'Content-Type':'application/json'}, body:JSON.stringify(body) });
...
function bindActionHandlers(data) {
  ...
  $("#editSRForm").off("submit").on("submit", async e => {
    ...
    // For backend, tweak 'custAction' below if needed to match your logic
